...
- price corridor entry gate: `BookState.Features` computes mean `mid` over `MEXC_SCALPER_PRICE_CORRIDOR_WINDOW` (optional weighted mean via `MEXC_SCALPER_PRICE_CORRIDOR_MEAN_HALF_LIFE`), bands from `MEXC_SCALPER_PRICE_CORRIDOR_PERCENTILE`, min samples `MEXC_SCALPER_PRICE_CORRIDOR_MIN_SAMPLES` (default 5; в коде значения ниже 2 приводятся к 2), зеркало ширины при односторонних отклонениях и при «пылевом» квантиле с одной стороны; `priceCorridorReject` сравнивает положение в **целых шагах цены**; фильтр привязан к **стороне сигнала книги** (согласовано с микроценой; `InvertExecution` не меняет полосу). Перелёт за `MEXC_SCALPER_PRICE_CORRIDOR_MAX_MULTIPLIER` по-прежнему отсекает вход.
...
- MEXC futures submit responses: if JSON has `success:false` (e.g. code 2005 balance), log `[trade] submit REJECTED ...`, emit `submit_error` / `flatten_error` with raw body, return error from `PlaceEntry`/`PlaceExit`/`EmergencyFlatten`; `placeNewStep` logs and continues without tearing down the WS session (лимитный entry-reprice цикл удалён)
